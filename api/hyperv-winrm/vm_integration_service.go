...
$vmIntegrationServicesObject = @(Get-ClusterGroup | ? {$_.GroupType -eq 'VirtualMachine' -and $_.Name -eq '{{.VmName}}' } | Get-VM | Get-VMIntegrationService | %{ @{
	Name=$_.Name;
	Enabled=$_.Enabled;
}})
...
Get-ClusterGroup | ? {$_.GroupType -eq 'VirtualMachine' -and $_.Name -eq '{{.VmName}}' } | Get-VM | Enable-VMIntegrationService -Name '{{.Name}}'
`))
...
Get-ClusterGroup | ? {$_.GroupType -eq 'VirtualMachine' -and $_.Name -eq '{{.VmName}}' } | Get-VM | Disable-VMIntegrationService -Name '{{.Name}}'
`))
